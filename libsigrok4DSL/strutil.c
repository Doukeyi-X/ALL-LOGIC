...
/*
 * Format whole.frac with SI prefix; strip trailing zeros in the fraction.
 * rem is x % base (0 < rem < base). e.g. base=1e6, rem=500000 → "5"
 * so 187 + "5" → "187.5 MHz" instead of "187.500000 MHz".
 */
static char *sr_si_format_frac(uint64_t whole, uint64_t rem, uint64_t base,
			       const char *si_prefix, const char *unit)
{
	char frac[32];
	int width = 0;
	uint64_t t;
	...
	if (unit == NULL)
		unit = "";
	if (rem == 0 || base <= 1)
		return g_strdup_printf("%llu %s%s", (u64_t)whole, si_prefix, unit);

	/* number of decimal digits = log10(base) */
	for (t = base; t > 1; t /= 10)
		width++;
	if (width <= 0 || width >= (int)sizeof(frac))
		return g_strdup_printf("%llu %s%s", (u64_t)whole, si_prefix, unit);

	/* zero-pad fraction, then drop trailing zeros */
	snprintf(frac, sizeof(frac), "%0*llu", width, (u64_t)rem);
	len = (int)strlen(frac);
	...
	return g_strdup_printf("%llu.%s %s%s", (u64_t)whole, frac,
			       si_prefix, unit);
}
 ...
 * Fractional parts never keep trailing zeros (187.5 MHz, not 187.500000).
 *
...
SR_API char *sr_si_string_u64(uint64_t x, const char *unit)
{
	...
	if (x >= SR_GHZ(1)) {
		return sr_si_format_frac(x / SR_GHZ(1), x % SR_GHZ(1),
					 SR_GHZ(1), "G", unit);
	} else if (x >= SR_MHZ(1)) {
		return sr_si_format_frac(x / SR_MHZ(1), x % SR_MHZ(1),
					 SR_MHZ(1), "M", unit);
	} else if (x >= SR_KHZ(1)) {
		return sr_si_format_frac(x / SR_KHZ(1), x % SR_KHZ(1),
					 SR_KHZ(1), "k", unit);
	} else {
