...
SR_PRIV int dsl_dev_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data)
{
    ...
    if (!devc->abort) {
        ...
        /*
         * Cancel outstanding bulk transfers so freewheel / poll loops can
         * drain quickly and still reach finish_acquisition() → SR_DF_END.
         * Without cancel, infinite-timeout transfers may hang forever on stop.
         */
        if (devc->transfers) {
        ...
        if (devc->submitted_transfers == 0 && devc->status != DSL_FINISH)
