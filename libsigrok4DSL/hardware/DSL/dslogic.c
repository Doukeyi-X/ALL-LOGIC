...
static void remove_sources(struct DSL_context *devc)
{
    ...
    if (devc->usbfd[0] == -1) {
        /* Timer/freewheel source used when libusb has no pollfds (e.g. Windows). */
        sr_source_remove(-1);
    } else {
...
static int dev_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
     ...
     * Note: libusb_get_pollfds() is not supported on Windows and returns NULL.
     * Fall back to a freewheel timer source (same approach as the demo driver).
     */
    ...
    if (!lupfd) {
        ...
        sr_info("%s: libusb has no pollfds, use freewheel source.", __func__);
        sr_source_add(-1, 0, 0, receive_data, sdi);
        devc->usbfd[0] = -1;
    } else {
        for (i = 0; lupfd[i]; i++);
