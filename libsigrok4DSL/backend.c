...
SR_PRIV int sr_listen_hotplug(struct sr_context *ctx, hotplug_event_callback callback)
{
    ...
    /* Prefer libusb hotplug when available (Linux). Windows often has none;
     * lib_main hotplug thread then uses poll_usb_hotplug_fallback(). */
    if (libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
	    ret = libusb_hotplug_register_callback(ctx->libusb_ctx,
		    ...
		    &ctx->hotplug_handle);
	    if (LIBUSB_SUCCESS != ret) {
		    sr_warn("libusb hotplug register failed: %s (will poll)",
			    libusb_error_name(ret));
	    } else {
		    sr_info("libusb hotplug registered.");
	    }
    } else {
	    sr_info("libusb has no hotplug capability; using poll fallback.");
    }
