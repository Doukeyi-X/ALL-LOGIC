...
{
    QDir dir(GetAppDataDir());
    QString notice = dir.absolutePath() + "/NOTICE.txt";
    if (QFile::exists(notice))
        QDesktopServices::openUrl(QUrl::fromLocalFile(notice));
    else
        MsgBox::Show("",
            "ALL LOGIC is an unofficial modified DSView build (GPLv3+).\n"
            "Do not file issues on the official DreamSourceLab project.",
            this);
}
 ...
 {
     MsgBox::Show("",
         "ALL LOGIC is an unofficial community build.\n"
         "It is not updated from the official DreamSourceLab website.",
         this);
 }
