...
{ 
   ...
   _is_able_drag = true;
    ...
    _title->setContentsMargins(28, 0, 0, 0);
...
{ 
    ...
    QPixmap logo(":/icons/alllogic_icon.png");
    if (logo.isNull())
        logo = QPixmap(":/icons/logo.svg");
    if (!logo.isNull()) {
        int side = height() - 6;
        QPixmap s = logo.scaled(side, side, Qt::KeepAspectRatio, Qt::SmoothTransformation);
        p.drawPixmap(6, (height() - s.height()) / 2, s);
    }
