...
{  
    ...
    if (!_mcp)
        _mcp = new pv::McpServer(NULL);
    ApplyMcp(AppConfig::Instance().appOptions.mcpEnabled,
             AppConfig::Instance().appOptions.mcpPort);
...
QString AppControl::McpStatusText() const
{
    if (_mcp)
        return _mcp->status_text();
    return QString("MCP  offline");
}

void AppControl::ApplyMcp(bool enabled, int port)
{
    if (!_mcp)
        _mcp = new pv::McpServer(NULL);
    if (!enabled) {
    ...
    if (port < 1024 || port > 65535)
        port = pv::McpServer::DefaultPort;
    if (_mcp->is_running() && _mcp->port() == port)
        return;
    _mcp->start(port);
}
